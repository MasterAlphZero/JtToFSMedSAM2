...
def mask_to_box(masks: jt.Var):
    ...
    min_xs, _ = jt.min(jt.where(masks, grid_xs, w).flatten(-2), dim=-1)
    max_xs, _ = jt.max(jt.where(masks, grid_xs, -1).flatten(-2), dim=-1)
    min_ys, _ = jt.min(jt.where(masks, grid_ys, h).flatten(-2), dim=-1)
    max_ys, _ = jt.max(jt.where(masks, grid_ys, -1).flatten(-2), dim=-1)
    bbox_coords = jt.stack((min_xs, min_ys, max_xs, max_ys), dim=-1)

    return bbox_coords


def _load_img_as_Var(img_path, image_size):
    ...
    img = jt.array(img_np).permute(2, 0, 1)
...
class AsyncVideoFrameLoader:
    ...
    def __getitem__(self, index):
        ...
        img, video_height, video_width = _load_img_as_Var(
...
def load_video_frames_from_jpg_images(
    ...
    images = jt.zeros((num_frames, 3, image_size, image_size))
    for n, img_path in enumerate(tqdm(img_paths, desc="frame loading (JPEG)")):
        images[n], video_height, video_width = _load_img_as_Var(img_path, image_size)
...
def load_video_frames_from_video_file(
    ...
    decord.bridge.set_bridge("jittor")
    ...
    images = jt.stack(images, dim=0).float() / 255.0
    # normalize by mean and std
    images -= img_mean
    images /= img_std
    return images, video_height, video_width
