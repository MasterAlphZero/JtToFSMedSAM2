...
class TwoWayAttentionBlock(nn.Module):
    ...
    def execute(
        self, queries: jt.Var, keys: jt.Var, query_pe: jt.Var, key_pe: jt.Var
    ) -> Tuple[jt.Var, jt.Var]:
...
class RoPEAttention(Attention):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.freqs_cis = (
            freqs_cis.to("cuda") if jt.compiler.has_cuda else freqs_cis
        )
        self.rope_k_repeat = rope_k_repeat

    def execute(
        self, q: jt.Var, k: jt.Var, v: jt.Var, num_k_exclude_rope: int = 0
    ) -> jt.Var:
        # Input projections
        q = self.q_proj(q)
        k = self.k_proj(k)
