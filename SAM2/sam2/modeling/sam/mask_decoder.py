...
class MaskDecoder(nn.Module):
    ...
    def execute(
        ...
        repeat_image: bool,
        high_res_features: Optional[List[jt.Var]] = None,
    ) -> Tuple[jt.Var, jt.Var]:
        """
    ...
    def _get_stability_scores(self, mask_logits):
        ...
        area_i = jt.sum(mask_logits > stability_delta, dim=-1).float()
        area_u = jt.sum(mask_logits > -stability_delta, dim=-1).float()
        stability_scores = jt.where(area_u > 0, area_i / area_u, 1.0)
    ...
    def _dynamic_multimask_via_stability(self, all_mask_logits, all_iou_scores):
        ...
        best_scores_inds = jt.argmax(multimask_iou_scores, dim=-1)
        batch_inds = jt.arange(
            multimask_iou_scores.size(0)
        )
        ...
        mask_logits_out = jt.where(
            is_stable[..., None, None].expand_as(singlemask_logits),
            singlemask_logits,
            best_multimask_logits,
        )
        iou_scores_out = jt.where(
