    ...
    with jt.no_grad():
        ...
        for curr_lb in test_labels:
            ...
            for sample_batched in testloader:
                test_indx += 1
                # print(f"{curr_lb_idx}_{test_indx}")
                ...
                query_labels = jt.concat([sample_batched['label'].cuda()], dim=0)

                # [way, [part, [shot x C x H x W]]] ->
                sup_img_part = [[shot_var.unsqueeze(0) for shot_var in support_images[0][q_part]]]   # way(1) x shot x [B(1) x C x H x W]
                sup_fgm_part = [[shot_var.unsqueeze(0) for shot_var in support_fg_mask[0][q_part]]]
                sup_bgm_part = [[shot_var.unsqueeze(0) for shot_var in support_bg_mask[0][q_part]]]
                ...
                query_pred = np.zeros_like(query_labels[0].cpu())
                ...
                if (sample_batched["z_id"] - sample_batched["z_max"] <= _config['z_margin']) and (sample_batched["z_id"] - sample_batched["z_min"] >= -1 * _config['z_margin']):
                    ...
                    # print(len(sup_img_part), len(sup_img_part[0]))
                    # print(len(query_images))
                    saved_dict = {'sup_img_part': sup_img_part[0][0].detach().cpu().numpy(), 'sup_fgm_part': sup_fgm_part[0][0].detach().cpu().numpy(), 'sup_bgm_part': sup_bgm_part[0][0].detach().cpu().numpy(), \
                                  'query_images': query_images[0].detach().cpu().numpy(), 'query_labels': query_labels.detach().cpu().numpy(), 'labels': [curr_lb], 'n_scan': curr_scan_count}
                    saved_path = os.path.join(saved_npz_path, f'{curr_lb_idx}_{test_indx}_case_{_scan_id}_z{sample_batched["z_id"][0]}.npz')
